...
const queryClient = new QueryClient({
  defaultOptions: {
    queries: {
      staleTime: 5 * 60 * 1000, // 5 minutos — evita refetches agressivos
...
const ProtectedRoute = ({ children, role }: { children: ReactNode, role?: string }) => {
  ...
  // Salva a rota atual para redirect pós-login
  if (!user) return <Navigate to="/auth" state={{ from: location.pathname }} />;
  
  // SUPERADMIN ou ADMIN tem acesso às rotas administrativas
  if (role === "SUPERADMIN" && isAdmin) return <>{children}</>;

  // SUPERADMIN tem acesso irrestrito a todas as áreas
