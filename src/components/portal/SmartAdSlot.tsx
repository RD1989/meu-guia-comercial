...
import { ShieldCheck, ArrowUpRight, X, MapPin, Zap } from "lucide-react";
...
export function SmartAdSlot({ type, city, className }: SmartAdSlotProps) {
  ...
  const targetCity = city || userLocation.city || "";
    ...
    async function fetchAds() {
      try {
        const { data, error } = await supabase
          .from("business_ads")
          .select("*, businesses(name, whatsapp)")
          .eq("type", type)
          .eq("status", "active");
        ...
        const filtered = data.filter((ad: any) => {
          const cityMatch = !ad.city || !targetCity || ad.city.toLowerCase() === targetCity.toLowerCase();
          ...
          if (ad.latitude && ad.longitude && !userLocation.loading && userLocation.lat) {
            ...
            distanceMatch = dist <= (ad.radius || 10000) / 1000;
          }
        ...
        console.error("Error fetching smart ads:", err);
        setCurrentAd(null);
      }
    }
