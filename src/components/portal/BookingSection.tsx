...
export const BookingSection: React.FC<BookingSectionProps> = ({ services = [], businessName, whatsapp }) => {
  ...
  const handleBooking = () => {
    ...
    if (url && url !== "#") {
      ...
      toast.error("WhatsApp do estabelecimento não disponível para agendamento.");
    }
  };
  ...
  if (!services || services.length === 0) {
    return (
      <div className="text-center py-16 px-4 bg-slate-50 rounded-[2.5rem] border border-dashed border-slate-200 mt-8 space-y-3">
        ...
        <h3 className="font-bold text-slate-700 text-sm uppercase tracking-wider">Nenhum serviço disponível para agendamento</h3>
        <p className="text-xs text-slate-400 max-w-sm mx-auto">Este estabelecimento ainda não cadastrou horários de agendamento online. Entre em contato direto pelo WhatsApp.</p>
      </div>
    );
  }
