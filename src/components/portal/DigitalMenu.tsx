...
import { Plus, Minus, ShoppingCart, Send, ChevronRight, X, Utensils } from "lucide-react";
...
export const DigitalMenu: React.FC<DigitalMenuProps> = ({ products = [], businessName, whatsapp }) => {
  ...
  const handleCheckout = () => {
    ...
    const url = formatWhatsAppUrl(whatsapp, message);
    if (url && url !== "#") {
      window.open(url, "_blank");
    } else {
      toast.error("WhatsApp do estabelecimento não disponível para pedidos.");
    }
  };

  if (!products || products.length === 0) {
    return (
      <div className="text-center py-16 px-4 bg-slate-50 rounded-[2.5rem] border border-dashed border-slate-200 mt-8 space-y-3">
        <Utensils className="h-10 w-10 text-slate-300 mx-auto" />
        <h3 className="font-bold text-slate-700 text-sm uppercase tracking-wider">Cardápio em atualização</h3>
        <p className="text-xs text-slate-400 max-w-sm mx-auto">Este estabelecimento ainda não cadastrou produtos em seu cardápio digital. Entre em contato direto pelo WhatsApp para solicitar o catálogo.</p>
      </div>
    );
  }
