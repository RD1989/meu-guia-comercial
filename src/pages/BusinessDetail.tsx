...
const BusinessDetail = () => {
  ...
  const { isFavorited, toggleFavorite } = useFavorites(business?.id);

  // Mutação para o Lojista Responder a Avaliação
  const replyReviewMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      setReplyTextMap(prev => ({ ...prev, [variables.reviewId]: "" }));
    },
    onError: (err: any) => {
      toast.error("Erro ao enviar resposta: " + err.message);
    },
    onSettled: () => setSubmittingReplyId(null)
  });

  const handleSendOwnerReply = (reviewId: string) => {
    const text = replyTextMap[reviewId];
    if (!text?.trim()) return;
    setSubmittingReplyId(reviewId);
    replyReviewMutation.mutate({ reviewId, reply: text });
  };
  ...
  // Increment profile views safely via RPC (SECURITY DEFINER) with fallback
  useEffect(() => {
    if (business?.id) {
      (supabase as any).rpc('increment_business_views', { b_id: business.id })
        .then(({ error }: any) => {
          if (error) {
            supabase.from("businesses").update({ profile_views: (business.profile_views || 0) + 1 }).eq("id", business.id).then(() => {});
          }
        });
    }
  }, [business?.id]);
  ...
  const handleWhatsAppClick = async () => {
    if (!business) return;
    try {
      await (supabase as any).rpc('increment_whatsapp_clicks', { b_id: business.id });
    } catch (_) {
      try {
    ...
    const url = formatWhatsAppUrl(business.whatsapp || business.phone, message);
    if (url && url !== "#") {
      window.open(url, "_blank");
    } else {
      toast.error("WhatsApp do estabelecimento não disponível.");
    }
  };
  ...
  const handleShare = async () => {
    ...
    const shareData = {
      title: business.name,
      text: `Confira ${business.name} no Guia Comercial!`,
    ...
    if (navigator.share) {
      ...
      toast.success("Link do estabelecimento copiado para a área de transferência!");
    }
  };
  ...
  // Real products and services without fake mock injection
  const displayProducts = products || [];
  const displayServices = services || [];

  // Schema.org LocalBusiness Structured Data for Google SEO
  const businessSchema = {
    "@context": "https://schema.org",
    "@type": "LocalBusiness",
    "name": business.name,
    "description": business.description || `Conheça ${business.name} no guia comercial local.`,
    "image": business.image_url || undefined,
    "telephone": business.phone || business.whatsapp || undefined,
    "address": {
      "@type": "PostalAddress",
      "streetAddress": business.address || "",
      "addressLocality": (business as any).city || "",
      "addressRegion": (business as any).state || "SP",
      "addressCountry": "BR"
    },
  ...
  return (
    <div className="min-h-screen bg-white pb-20 md:pb-0">
      <SEO 
        title={business.name}
        description={business.description || `Veja telefone, endereço, cardápio e avaliações de ${business.name}.`}
      ...
      <div className="relative h-72 md:h-96 lg:h-[450px] group overflow-hidden">
        ...
        <div className="absolute bottom-16 left-0 w-full px-8 z-20">
          <div className="max-w-5xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-8">
            ...
            {/* Premium CTA Actions */}
            <div className="flex flex-wrap items-center gap-3 bg-white/5 backdrop-blur-2xl p-3 rounded-[2.5rem] border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.3)]">
              <Button 
              ...
              <div className="flex gap-2">
                <Button 
                  ...
                  variant="outline" 
                  className={`rounded-2xl h-14 w-14 p-0 backdrop-blur-md transition-all ${
                    isFavorited 
                ...
                <Button 
                  onClick={handleShare}
                  variant="outline" 
                  className="rounded-2xl h-14 w-14 p-0 bg-white/10 text-white border-white/10 hover:bg-white/20 hover:border-white/30 transition-all"
                  ...
                  <div className="space-y-4">
                    {reviews.length === 0 ? (
                      ...
                      reviews.map((review) => {
                        ...
                        const isReplyingThis = submittingReplyId === review.id;
                        const replyDraft = replyTextMap[review.id] ?? "";
                        ...
                        return (
                          <Card key={review.id} className="p-8 border-none shadow-xl shadow-slate-200/40 rounded-[2.5rem] bg-white space-y-4">
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <div className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">
                                    {format(new Date(review.created_at), "d MMMM yyyy", { locale: ptBR })}
                                  </div>
                                </div>
                              </div>
                            ...
                            <p className="text-sm text-slate-600 font-medium leading-relaxed italic">
                              "{review.comment || "Excelente estabelecimento!"}"
                            </p>
                              ...
                              <div className="mt-4 p-4 rounded-2xl bg-slate-50 border border-slate-100 flex items-start gap-3">
                                ...
                                <div className="space-y-1 flex-1">
                                  <div className="flex items-center gap-2">
                                    <span className="text-xs font-black text-slate-900">Resposta do Estabelecimento</span>
                                    {review.owner_reply_at && (
                                      <span className="text-[9px] text-slate-400 font-bold uppercase">
                                        • {format(new Date(review.owner_reply_at), "d/MM/yyyy")}
                                      </span>
                                    )}
                                  </div>
                            ...
                            {/* Campo de Resposta exclusivo para o Dono do Negócio */}
                            {isOwner && (
                              <div className="pt-2 border-t border-slate-100">
                                {!review.owner_reply ? (
                                  <div className="flex items-center gap-2 mt-2">
                                    <Input
                                      placeholder="Responder publicamente a este cliente..."
                                      value={replyDraft}
                                    ...
                                    <Button
                                      size="sm"
                                      onClick={() => handleSendOwnerReply(review.id)}
                                      disabled={isReplyingThis || !replyDraft.trim()}
                                      className="h-10 px-4 rounded-xl text-xs font-bold bg-primary hover:bg-primary/90 text-white shrink-0 gap-1.5"
                                    >
                                      {isReplyingThis ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Send className="h-3.5 w-3.5" />}
                                      Responder
                                    </Button>
                                  </div>
                                ) : (
                                  <details className="text-xs text-slate-400 group/edit">
                                    <summary className="cursor-pointer font-bold hover:text-primary list-none text-[10px] uppercase tracking-wider">
                                      ✏️ Editar resposta
                                    </summary>
                                    <div className="flex items-center gap-2 mt-2">
                                      <Input
                                        placeholder="Atualizar sua resposta..."
                                        value={replyDraft || review.owner_reply}
                                        onChange={(e) => setReplyTextMap(prev => ({ ...prev, [review.id]: e.target.value }))}
                                        className="h-10 text-xs rounded-xl bg-slate-50 border-slate-200"
                                        disabled={isReplyingThis}
                                      />
                                      <Button
                                        size="sm"
                                        onClick={() => handleSendOwnerReply(review.id)}
                                        disabled={isReplyingThis || !(replyDraft || review.owner_reply).trim()}
                                        className="h-10 px-4 rounded-xl text-xs font-bold bg-primary hover:bg-primary/90 text-white shrink-0"
                                      >
                                        {isReplyingThis ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Salvar"}
                                      </Button>
                                    </div>
                                  </details>
                                )}
