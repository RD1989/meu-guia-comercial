...
export default function AdminBlogCategories() {
  ...
  const { data: categories, isLoading } = useQuery({
    queryKey: ["admin-blog-categories", searchTerm],
    queryFn: async () => {
      // NOTE: Using 'blog_categories' table. 
      // Falling back to a selection from 'blog_posts' if table not yet created 
      // to avoid UI crash, but primary target is 'blog_categories'.
      let query = (supabase as any).from("blog_categories").select("*");
      if (searchTerm) {
    ...
    <AdminLayout>
      <div className="p-8 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-slate-800">Categorias do Blog</h1>
            <p className="text-slate-500">Gerencie os tópicos para organizar seus artigos e notícias.</p>
          </div>
            ...
            <DialogContent className="sm:max-w-[425px]">
              ...
              <div className="grid gap-4 py-4">
                <div className="space-y-2">
                  ...
                  <Input id="name" value={catName} onChange={(e) => setCatName(e.target.value)} placeholder="Ex: Gastronomia, Eventos..." />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="slug">Slug (URL)</Label>
                  <Input id="slug" value={catSlug} onChange={(e) => setCatSlug(e.target.value)} placeholder="ex-gastronomia" />
                </div>
              </div>
              <DialogFooter>
