...
const Search = () => {
  ...
  const cityParam = searchParams.get("cidade") || currentCity || "";
  ...
  const { data: businesses = [], isLoading } = useQuery({
    queryKey: ["search-businesses", selectedCategory, queryText, cityParam],
    queryFn: async () => {
      let q = supabase
        .from("businesses")
      ...
      if (cityParam && cityParam !== "Todas as Cidades") {
        q = q.ilike("city", `%${cityParam}%`);
      }
  ...
  const pageTitle = queryText 
    ? `Busca por "${queryText}" em ${cityParam || "sua região"}`
    : selectedCategory
    ? `Empresas em ${cityParam || "sua região"}`
    : `Explorar Empresas em ${cityParam || "sua região"}`;

  return (
    <div className="min-h-screen bg-background pb-20 md:pb-0">
      <SEO 
        title={pageTitle}
        description={`Encontre as melhores empresas, telefones, endereços e avaliações em ${cityParam || "sua cidade"}.`}
      />
      <Header />
