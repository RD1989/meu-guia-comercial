...
export default function DashboardCoupons() {
  ...
  const handleRedeemCoupon = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!redeemCode.trim() || !business?.id) return;

    setIsValidating(true);
    try {
      // Buscar cupom pelo código vinculado a este negócio
      const { data: matchedCoupons, error } = await (supabase as any)
        .from("coupons")
        .select("*")
        .eq("business_id", business.id)
      ...
      if (!matchedCoupons || matchedCoupons.length === 0) {
        toast.error("Cupom não encontrado ou não pertence a este estabelecimento.");
        return;
      }

      const coupon = matchedCoupons[0];

      if (!coupon.active) {
        toast.error("Este cupom está desativado.");
        return;
      }

      if (isExpired(coupon.valid_until)) {
        toast.error("Este cupom expirou a validade.");
        return;
      }

      if (coupon.max_uses && coupon.current_uses >= coupon.max_uses) {
        toast.error("Este cupom atingiu o limite máximo de utilizações.");
        return;
      }
      ...
      const discountMsg = coupon.discount_type === "percent" 
        ? `${coupon.discount_value}% de desconto`
        : coupon.discount_type === "fixed"
        ? `R$ ${coupon.discount_value} de desconto`
        : "Brinde / Grátis";

      toast.success(`🎉 Cupom "${coupon.code}" VALIDADO! Benefício: ${discountMsg}`, {
        duration: 5000
      });
  ...
  return (
    <DashboardLayout title="Cupons & Promoções">
      <div className="space-y-6">
        ...
        {/* ⚡ Card de Validação no Balcão */}
        <Card className="border-none shadow-xl bg-gradient-to-r from-slate-900 to-slate-800 text-white rounded-[2.5rem] p-6 sm:p-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-1">
              <Badge className="bg-primary text-white border-none font-black text-[9px] uppercase tracking-wider mb-2">
                Ponto de Venda
              </Badge>
              <h3 className="text-2xl font-black tracking-tight">Validar Cupom no Balcão</h3>
              <p className="text-xs text-slate-400 font-medium max-w-md">
                O cliente apresentou um código de cupom? Digite-o abaixo para registrar a venda e dar baixa.
              </p>
            </div>

            <form onSubmit={handleRedeemCoupon} className="flex items-center gap-3 w-full md:w-auto">
              <Input 
                value={redeemCode}
                onChange={(e) => setRedeemCode(e.target.value.toUpperCase())}
                placeholder="Ex: DESC-10"
                className="h-12 bg-white/10 border-white/15 text-white font-mono font-black text-sm uppercase rounded-2xl placeholder:text-slate-500 w-full md:w-48 text-center"
              />
              <Button 
                type="submit"
                disabled={isValidating || !redeemCode.trim()}
                className="h-12 px-6 rounded-2xl bg-emerald-500 hover:bg-emerald-600 text-white font-black text-xs uppercase tracking-wider shadow-lg shadow-emerald-500/20 shrink-0"
              >
